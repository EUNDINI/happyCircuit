...
public class MusicDAO {
	...
	public int createMusic(Music music) throws Exception {
		 ...
		 * artistId VARCHAR2(30) NULL , 
		 * musicName VARCHAR2(20) NULL , 
		 * genre VARCHAR2(20) NULL , 
		 * musicPath VARCHAR2(20) NULL ,
		 * originalMusicId NUMBER NULL , 
		 * priorMusicId NUMBER NULL , 
		 * nth NUMBER NULL
		 */

		Object[] param;
		String sql;
		if (music.getOriginalMusicId() != 0) {
			sql = "INSERT INTO Music VALUES (music_seq.nextval,?,?,?,?,?,?,?)";
			param = new Object[] { music.getArtistId(), music.getMusicName(), music.getGenre(),
					music.getMusicPath(), music.getOriginalMusicId(), music.getPriorMusicId(), music.getNth() };
		} else {
			sql = "INSERT INTO Music VALUES (music_seq.nextval,?,?,?,?,null,null,?)";
			param = new Object[] { music.getArtistId(), music.getMusicName(), music.getGenre(),
					music.getMusicPath(), music.getNth() };
		}
	...
	public Music findMusic(int musicId) throws Exception {
		String sql = "SELECT NVL(originalMusicId,0), NVL(priorMusicId,0), artistId, musicName, genre, nth, musicPath FROM Music WHERE musicId = ?";
		...
		try {
			...
			if (rs.next()) {
				music = new Music(musicId, rs.getInt("originalMusicId"), rs.getInt("priorMusicId"),
						rs.getString("artistId"), rs.getString("musicName"), rs.getString("genre"),
						rs.getInt("nth"), rs.getString("musicPath"));
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		} finally {
	...
	public List<LikeChart> getLikeChart(String genre) throws Exception {
		...
		if (genre.equals("all")) {
			...
			sql = "SELECT musicId, likeCount, regDate, ROWNUM AS ranking FROM MusicArticle a, MUSIC m where a.musicId = m.musicId and"
					+ "m.genre = ? and  ROWNUM <= 10 ORDER BY likeCount, regDate";
		...
		try {
			...
			while (rs.next()) {
				...
				Music music = findMusic(rs.getInt("musicId"));
				likeChart = new LikeChart(music.getMusicId(), rs.getInt("ranking"), rs.getInt("likeCount"),
						music.getMusicName(), music.getArtistId(), rs.getDate("regDate"));
